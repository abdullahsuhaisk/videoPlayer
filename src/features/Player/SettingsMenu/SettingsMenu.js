...
const SettingsMenu = () => {
  ...
  const handlePlayerStateChange = useCallback((event) => {
    ...
    setPanelStyle({
      maxHeight: `${context.player.currentHeight() - offset}px`
    });
  }, []);
  ...
  if (activeMenu === 'main-menu') {
    MenuComponent = (
      <MainMenu
        ...
        onSelectMenu={(selectedMenu) => handleSelectedMenu(selectedMenu)}
      />
    );
    ...
    MenuComponent = (
      <PlaybackRateMenu
        title="Speed"
        onSelectRate={(selectedRate) => handleSelectedRate(selectedRate)}
